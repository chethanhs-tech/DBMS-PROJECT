...
async function run() {
  ...
  for (let i = 0; i < toFix.length; i++) {
    ...
    try {
      const prompt = `Professional studio product photography of ${p.product_name}, grocery item, high quality, clean white background, 4k`;
      const aiUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=600&height=600&nologo=true&seed=${Math.floor(Math.random()*10000)}`;
